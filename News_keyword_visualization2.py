...
for word in keywords:
    ...
    encText2 = urllib.parse.quote("1")  #찾을 검색 결과 수
    ...
    if(rescode==200): #네이버API와 응답 확인용
        response_body = response.read() #API응답을 바이트 형태로 받음(JSON 문자열)
        response_dict = json.loads(response_body) # json형식의 문자열을 파이썬 객체(딕셔너리)로 전환
...
plt.axis('off') #matplotlib 그래프 축 숨기기
plt.show()
plt.savefig("뉴스 키워드 워드클라우드.png") #이미지 저장
